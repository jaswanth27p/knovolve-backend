...
def require_started(db: Session, user_id: int, course: Course) -> None:
    enrollment = db.scalar(
        select(UserCourse).where(UserCourse.user_id == user_id, UserCourse.course_id == course.id)
    )
    if enrollment is None:
        raise ChatToolError(f"You haven't started the course '{course.topic_raw}' yet.")


def require_started_course(db: Session, user_id: int, course_slug: str) -> Course:
