...
def _get_or_dispatch_chapter_assignment(db: Session, content: ChapterContent) -> AssignmentResponse:
    ...
    if content.status != "ready":
        # Don't 404 here: the relevant version can be mid-remediation (its
        # content is generated by a background task after a failed attempt).
        # Report the state so the UI shows a spinner instead of a hard error.
        if content.status == "failed":
            return AssignmentResponse(status="failed", error=content.error or "Chapter generation failed.")
