...
- Infra: `docker compose up -d` (Postgres + Redis + MinIO). The six `langfuse-*` services sit behind a compose profile and are NOT started by that command — they need the `LANGFUSE_*` secrets in `.env` (see `.env.example`) and would crash-loop without them. Start them with `docker compose --profile langfuse up -d` (the `--profile` flag is a top-level flag, before the subcommand) when you need the Langfuse UI/ingest at `http://localhost:3400`.
